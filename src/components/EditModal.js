...
function EditModal({ closeAdd, singleUser }) {
    const [name, setName] = useState(singleUser.name);
    const [country, setCountry] = useState(singleUser.country);
    const [address, setAddress] = useState(singleUser.address);
    const [job, setJob] = useState(singleUser.job_title);
    const [phone, setPhone] = useState(singleUser.phone_number);
    const [status, setStatus] = useState(singleUser.status);
    ...
    const handleUpdate = () => {
        ...
        axios.put('https://mitramas-test.herokuapp.com/customers', body, {
            headers: {
        ...
        <div className='absolute bg-black bg-opacity-50 flex justify-center items-center inset-0'>
            <div className='bg-slate-200 py-2 px-3 rounded-lg shadow-xl w-[380px]'>
                <div className='flex justify-between items-center px-4 mb-5 mt-2'>
                    <div></div>
                    <h1 className='text-center text-xl font-semibold'>Edit User</h1>
                ...
                <button className='w-full bg-blue-500 text-white py-2 rounded-lg hover:opacity-80 disabled:opacity-80' onClick={() => handleUpdate()}>Submit</button>
            </div>
        </div>
    )
}
